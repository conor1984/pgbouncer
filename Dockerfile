...
RUN sudo apt-get update &&\ 
    ...
    libxslt-dev libxml2-dev libpam-dev libedit-dev flex bison git &&\
    pgbouncer repmgr 
...
#SSH
RUN sudo chown maximus.maximus authorized_keys id_rsa.pub id_rsa &&\
...
RUN pg_ctl start -l $PGLOG &&\
	...
	repmgr -d Repmgr -U repmgr -d Repmgr -h pgbouncer -D $PGDATA -f $PGREP/repmgr.conf witness create &&\
